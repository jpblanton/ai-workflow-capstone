...
def _model_train(df,tag,test=False):
    ...
    grid.fit(X, y)
    model_name = re.sub(os.getcwd(),"_",str(MODEL_VERSION))
    if test:
...
def model_load(prefix='sl',data_dir=None,training=True):
    ...
    models = [f for f in os.listdir(os.path.join(".","models")) if f.endsWith('.csv')]
...
if __name__ == "__main__":
    ...
    data_dir = os.path.join(os.getcwd(), "cs-train")
